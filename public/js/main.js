...
function deleteFoodClicked(id) {
	if (confirm("Are you sure you want to delete this? !! THIS IS IRREVERSIBLE !!")) {
		fetch(`http://admin.smokinbsbbq.tk/d/food/${id}`).then(() => {
			location.reload();
			console.log("Deleted");
		})
	}
...
function deleteAlertClicked(id) {
	if (confirm("Are you sure you want to delete this? !! THIS IS IRREVERSIBLE !!")) {
		fetch(`http://admin.smokinbsbbq.tk/d/alert/${id}`).then(() => {
			location.reload();
			console.log("Deleted");
		})
	}
...
function forgot() {
	alert("Key: (smokinbs) Pass: (current[5+4]). It is recommended you let your browser save this!")
}
